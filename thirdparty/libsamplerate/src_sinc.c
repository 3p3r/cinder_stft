...
{	SINC_FILTER *filter, temp_filter ;
	...
	*/
	...
	if ((filter = static_cast<SINC_FILTER*>(calloc (1, sizeof (SINC_FILTER) + sizeof (filter->buffer [0]) * (temp_filter.b_len + temp_filter.channels)))) == NULL)
...
{	double		fraction, left [2], right [2], icoeff ;
	...
	output [0] = static_cast<float>(scale * (left [0] + right [0])) ;
	output [1] = static_cast<float>(scale * (left [1] + right [1])) ;
} /* calc_output_stereo */
...
{	double		fraction, left [4], right [4], icoeff ;
	...
	output [0] = static_cast<float>(scale * (left [0] + right [0])) ;
	output [1] = static_cast<float>(scale * (left [1] + right [1])) ;
	output [2] = static_cast<float>(scale * (left [2] + right [2])) ;
	output [3] = static_cast<float>(scale * (left [3] + right [3])) ;
} /* calc_output_quad */
...
{	double		fraction, left [6], right [6], icoeff ;
	...
	output [2] = static_cast<float>(scale * (left [2] + right [2])) ;
	output [3] = static_cast<float>(scale * (left [3] + right [3])) ;
	output [4] = static_cast<float>(scale * (left [4] + right [4])) ;
	output [5] = static_cast<float>(scale * (left [5] + right [5])) ;
} /* calc_output_hex */
...
{	double		fraction, icoeff ;
	...
	do
	{
		...
		{	default :
				ch -- ;
				output [ch] = static_cast<float>(scale * (left [ch] + right [ch])) ;
			case 7 :
				ch -- ;
				output [ch] = static_cast<float>(scale * (left [ch] + right [ch])) ;
			case 6 :
